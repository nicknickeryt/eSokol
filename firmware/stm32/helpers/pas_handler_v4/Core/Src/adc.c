...
uint16_t rawBatteryCurrentValues[ADC_AVG_SAMPLES] = {0};
...
void processBatteryCurrent() {
  uint16_t adcSum = 0;
  for (uint8_t i = 0; i < ADC_AVG_SAMPLES; i++)
     adcSum += rawBatteryCurrentValues[i];
   
  batteryCurrent = initialAdcMeasurementCompleted ? (1000.0f * (((adcSum/ADC_AVG_SAMPLES) * 3.3f) / 4096.0f)) : (1000.0f * ((rawAdcValues[ADC_BATTERY_CURRENT_CHANNEL] * 3.3f) / 4096.0f));
}
...
void processThrottleVoltage() {
  uint16_t adcSum = 0;
  for (uint8_t i = 0; i < ADC_AVG_SAMPLES; i++)
     adcSum += rawThrottleValues[i];
...
void processAdcMeasurement() {
  ...
  rawThrottleValues[adcIndex] = rawAdcValues[ADC_THROTTLE_AIN_CHANNEL];
  rawBatteryCurrentValues[adcIndex] = rawAdcValues[ADC_BATTERY_CURRENT_CHANNEL];
