...
{
	float rawDutyCycle = ALGORITHM_EQ_FACTOR * ((-0.00000044 * (x * x * x * x * x)) - (0.000049 * (x * x * x * x)) + (0.00164 * (x * x * x)) + (0.0169 * (x * x)) + (1.1815 * x) + 18.912 + ALGORITHM_EQ_COMPONENT); // DUTY_EQ_COMPONENT to spin a bit FASTER than pedals
	...
	else
	{
		...
		{ // speeding up
			targetDutyCycle = (DUTY_SMOOTH_FACTOR_UP * rawDutyCycle) + ((1 - DUTY_SMOOTH_FACTOR_UP) * previousDutyCycle);
		}
		else
		{ // slowing down
			targetDutyCycle = (DUTY_SMOOTH_FACTOR_DOWN * rawDutyCycle) + ((1 - DUTY_SMOOTH_FACTOR_DOWN) * previousDutyCycle);
		}
	}
...
bool runAlgorithm()
{
	...
	// If THR_DIS is set to high, then throttle is enabled, we don't need to run algorithm.
