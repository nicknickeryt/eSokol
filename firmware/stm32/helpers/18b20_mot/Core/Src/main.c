...
void calcAndSetDutyCycleFromTemp(float temp) {
	TIM1->CCR1 = (20.0*temp) - 400.0;
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		if (temp >= 80.0f)
			send_string("Sensor error...\r\n");
		else {
			calcAndSetDutyCycleFromTemp(temp);
			send_string("Temp: ");
			send_float(temp);
			send_string("\r\n");
			send_string("Duty cyle: ");
			send_float(TIM1->CCR1);
			send_string("\r\n");
		}
